...
import '../site.css'
import { getAnimalByName, getInformations, getPlacements, updateAnimal } from '../http/listApi'
  ...
  class AnimalForm extends React.Component {
    constructor(props) {
      ...
      this.state = {name: name, nameValid: nameIsValid,
                    ...
                    animal: {}, placements: [], informations: [], placement: 1, information: 1}
    ...
    handleSubmit(e) {
        ...
        if (nameValid) {
          getAnimalByName(name).then(data => {
            ...
            if (currentAnimal) {
              if (admissionValid) {
                currentAnimal.admission = admission
              }
              if (birthValid) {
                currentAnimal.birth = birth
              }
              if (imageValid) {
              ...
              currentAnimal.placementId = placement
              currentAnimal.informationId = information

              try {
                updateAnimal(currentAnimal).then(data => {
                  console.log(data)
                })
              } catch (e) {
                alert('Invalid data!')
    ...
    render() {
      ...
      return (
          <form>
            <p>Required field is only name, the other may be filled if you want to update this property!</p>
            <p>
